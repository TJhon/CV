...
- Julia for Data Science (Progreso)

\newpage


# Egresado de economía de la UNCP


```{r}
knitr::include_graphics("moi/capture.png")
```
...
```{r}
knitr::include_graphics("moi/ceu.png")
```
\newpage
...
```{r}
knitr::include_graphics("moi/kaisen.png")
```
\newpage
...
```{r}
f <- list.files(
  path = paste0(here("CV/certificados")), pattern = c(
    "(.*).pdf$"#, 
    #"(.*).PNG$"
    ), 
  full.names = T
